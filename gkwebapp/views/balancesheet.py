...
'''
This function returns a spreadsheet form of Statement of Sources and Applications of Funds Report.
        ...
        for record in sources:
            ...
            if record["groupAccflag"]==2 or record["groupAccflag"]==1:
                if record["advflag"]==1:
                    ...
                    sheet['B'+str(row)].font = Font(name='Liberation Serif',size='16',bold=True,color=RED)
                    
                else:
                    sheet['B'+str(row)] = record["amount"]
                    sheet['B'+str(row)].alignment = Alignment(horizontal = 'right')
                    sheet['B'+str(row)].font = Font(name='Liberation Serif',size='16',bold=False)
            elif (record["groupAccflag"]=="" and record["subgroupof"]!=""):
                if record["advflag"]==1:
                    sheet['C'+str(row)] = record["amount"]
                    sheet['C'+str(row)].alignment = Alignment(horizontal = 'right')
                    sheet['C'+str(row)].font = Font(name='Liberation Serif',size='16',bold=True,color=RED)
                else:
                    sheet['C'+str(row)] = record["amount"]
                    sheet['C'+str(row)].alignment = Alignment(horizontal = 'right')
                    sheet['C'+str(row)].font = Font(name='Liberation Serif',size='16',bold=False)
            else:
                if record["advflag"]==1:
                    sheet['D'+str(row)] = record["amount"]
                    sheet['D'+str(row)].alignment = Alignment(horizontal = 'right')
                    sheet['D'+str(row)].font = Font(name='Liberation Serif',size='16',bold=True,color=RED)
                else:
                    sheet['D'+str(row)] = record["amount"]
                    sheet['D'+str(row)].alignment = Alignment(horizontal = 'right')
                    sheet['D'+str(row)].font = Font(name='Liberation Serif',size='16',bold=False)
            row += 1
        for record in applications:
            if record["groupAccname"]=="Total" or record["groupAccname"]=="Sources:" or record["groupAccname"]=="Difference" :
                sheet['A'+str(row)] = record["groupAccname"].upper()
